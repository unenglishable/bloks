    ...
    HudRenderCallback.EVENT.register(
        (drawContext, tickDelta) -> {
          ...
          switch (corner) {
            ...
            case "top_right" -> {
              x = screenW - margin - textW;
              y = margin;
            }
            case "bottom_left" -> {
              x = margin;
              y = screenH - margin - textH;
            }
            default -> {
              x = screenW - margin - textW;
              y = screenH - margin - textH;
          ...
          drawContext.drawTextWithShadow(
              mc.textRenderer, net.minecraft.text.Text.literal(text), x, y, 0xFFFFFFFF);
        });
    ...
    ClientTickEvents.END_CLIENT_TICK.register(
        client -> {
          ...
          if (toggleKey != null && toggleKey.wasPressed()) {
            ...
            if (ClientState.get().isToastEnabled()) {
              Toasts.showDayToast(
                  state.getCurrentDay(), state.isHudVisible() ? state.getDayLabel() : "");
            }
          }
